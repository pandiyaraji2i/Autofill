//
//  IIFormTextField.swift
//  TestApp
//
//  Created by Pandiyaraj on 2/20/20.
//  Copyright © 2020 Ideas2IT. All rights reserved.
//

import UIKit

class IIFormTextField: UITextField {
  ...
  fileprivate final func setup() {
    ...
    self.lineColor = UIColor.init(hex: 0x000000, transparency: 0.12)
    self.selectedLineColor = UIColor.init(hex: 0x000000, transparency: 0.12)
    self.placeholderColor = UIColor.init(hex: 0x666666)
  ...
  fileprivate func createTitleLabel() {
    ...
    label.autoresizingMask = [.flexibleWidth, .flexibleHeight]
      label.font = UIFont.systemFont(ofSize: 15)// UIFont.systemFont(ofSize: 13)
    label.alpha = 0.0
  ...
  fileprivate func createErrorLabel() {
    ...
    label.frame = CGRect.init(x: frame.origin.x, y: frame.origin.y + 25, width: frame.size.width, height: frame.size.height)
  ...
  open func errorLabelRectForBounds(_ bounds: CGRect) -> CGRect {
    ...
    return CGRect(x: 0, y: (bounds.size.height - boundingRect.size.height) + 1, width: boundingRect.size.width, height: boundingRect.size.height)
  }
  ...
  open override var description: String {
    return "[IIFormTextField(\(String(describing: placeholder))) text:\(String(describing: text))]"
  }
